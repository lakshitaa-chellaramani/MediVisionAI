  ...
  return (
    <div className="bg-neutral-900 min-h-screen text-white">
      ...
      <div className="max-w-6xl mx-auto p-6">
        <h1 className="text-2xl font-bold mb-8 text-rose-400">Find Specialists</h1>
        
        <div className="bg-neutral-800 p-6 rounded-xl mb-8 shadow-lg">
          <h2 className="text-lg font-medium mb-4 text-rose-300">Filter Options</h2>
          ...
          <Button className="bg-rose-500 hover:bg-rose-600 text-white mt-4 w-full md:w-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          {filteredDoctors.map((doctor) => (
            <Card key={doctor.id} className="bg-neutral-800 border-none overflow-hidden rounded-xl shadow-lg hover:shadow-rose-900/20 transition-all duration-300">
              <div className="flex flex-col md:flex-row">
                <div className="md:w-1/3 p-4 flex justify-center items-center bg-neutral-700">
                  <img 
                    src={doctor.image} 
                    alt={doctor.name} 
                    className="w-32 h-32 object-cover rounded-full border-2 border-rose-500/20"
                  />
                </div>
                
                <div className="md:w-2/3 p-4">
                  <CardHeader className="p-0 pb-2">
                    <h3 className="text-xl font-semibold text-rose-400">{doctor.name}</h3>
                  ...
                  <CardFooter className="p-0 pt-3">
                    <Dialog>
                      <DialogTrigger asChild>
                        <Button 
                          className={`w-full ${doctor.available ? 'bg-rose-500 hover:bg-rose-600' : 'bg-neutral-700 cursor-not-allowed'}`} 
                      ...
                      <DialogContent className="bg-neutral-900 border-neutral-700 text-white">
                        <DialogHeader>
                          <DialogTitle className="text-rose-400">Book Appointment with {selectedDoctor?.name}</DialogTitle>
                        </DialogHeader>
                        <div className="space-y-4 py-4">
                          ...
                          <Button className="bg-rose-500 hover:bg-rose-600 text-white w-full mt-4">
                            Confirm Appointment
                          </Button>
                        </div>
                      </DialogContent>
        ...
        <div className="flex justify-center">
          <Pagination>
            ...
            <PaginationItem className="bg-rose-500 text-white">1</PaginationItem>
